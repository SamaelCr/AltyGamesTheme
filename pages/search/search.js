...
(function() {
    ...
    function performYTSearch(query) {
        if (resultsContainer) {
            resultsContainer.innerHTML = '<div class="search-message"><i class="fa-solid fa-circle-notch fa-spin"></i><br>Buscando "'+query+'"...</div>';
        }
        
        // Llamada a la API de Blogger usando el helper global getJSONP del padre
    ...
    window.renderYTSearch = function(json) {
        ...
        if (!json || !json.feed || !json.feed.entry || json.feed.entry.length === 0) {
            if (resultsContainer) {
                resultsContainer.innerHTML = '<div class="search-message"><i class="fa-regular fa-face-frown"></i><br>No se encontraron resultados para "'+currentQuery+'".</div>';
            }
            return;
        }
